...
respaldo (){
	...
	then
	        if ["$BSERVER" != "fourier2"];then
			rsync  $OPTS -e 'ssh -p 22' --link-dest=../backup.1 soporte@$BSERVER:/datos/$BSERVER $DESTINATION/backup.0/ | tee -a $logfile
		else	
			rsync  $OPTS -e 'ssh -p 22' --link-dest=../backup.1 soporte@fourier:/datos/$BSERVER $DESTINATION/backup.0/ | tee -a $logfile
		fi
		if [[ $? != 0 ]]
		then 
			echo "0" > $doBackupCheckFile
...
# /mnt/DroboFS/Shares/backup/./rotate_backups_fmrilab.sh
...
ssh soporte@talairach /home/inb/soporte/fmrilab-scripts/mantenimiento/./fmrilab_aviso_correo.sh -c nekrum@gmail.com -m "Started at  : $started 
